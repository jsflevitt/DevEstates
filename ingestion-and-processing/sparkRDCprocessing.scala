...
val rdchDF = spark.sql("""SELECT format_string("%05d",postal_code) AS zip_code,
								 to_date(concat_ws('-',substring(month_date_yyyymm,1,4),substring(month_date_yyyymm,5,2))) AS date,
